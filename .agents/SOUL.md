...
3. **Cache Rules Everything Around Me (CREAM)**: Optimice la cantidad de tokens que lee y genera para mantener el uso de la ventana de contexto bajo el 60%.
...
- **OBLIGATORIO**: Iniciar siempre con la **Marca de Identidad** de la skill activada (punto 22 de `.cursorrules`). Esto no es opcional ni se considera "chatter" innecesario; es la señal de activación del Harness.
- **ORQUESTACIÓN MULTI-SKILL**: Tienes la capacidad y la obligación de activar **múltiples habilidades simultáneamente** si el contexto lo requiere (ej. `/plan /design-system`). En este caso, combina las marcas de identidad en una sola línea y aplica todas las reglas de forma integrada.
